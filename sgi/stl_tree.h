...
__rb_tree_rebalance_for_erase(__rb_tree_node_base* z,   //z是需要删除的结点
                              __rb_tree_node_base*& root,
                              __rb_tree_node_base*& leftmost,
                              __rb_tree_node_base*& rightmost)
{
  __rb_tree_node_base* y = z;   //指向最终要删除的结点
  __rb_tree_node_base* x = 0;    
  __rb_tree_node_base* x_parent = 0;

  //只有一个孩子或者没有孩子的情况，用实际删除的y代替z
  if (y->left == 0)             // z has at most one non-null child. y == z.  //如果y没有左孩子
    x = y->right;               // x might be null.             //x指向它的的右孩子
  else                           
     if (y->right == 0)          // z has exactly one non-null child.  y == z.  //如果y没有右孩子，x指向它的左孩子
      x = y->left;              // x is not null.           
    else {                      // z has two non-null children.  Set y to     //上面那两种情况都不是，那y就有两个孩子
      y = y->right;             //   z's successor.  x might be null.
      while (y->left != 0)
        y = y->left;   //找到右子树的最左，中序遍历后继
      x = y->right;    //那么x就是该y的右子树
    }

//////////////////////////////////////////////////////////////////////////////////////////////////////////////
//y != z 说明有两个孩子非空,从上面得出的
  if (y != z) {                 // relink y in place of z.  y is z's successor 
    z->left->parent = y; 
    y->left = z->left;
    if (y != z->right) {
      x_parent = y->parent;    //这个x_parent不等于x->parent,因为x有可能是nil结点
      if (x) x->parent = y->parent;   //如果x不是nil节点，更新x父结点
      y->parent->left = x;      // y must be a left child
      y->right = z->right;
      z->right->parent = y;   //记住z的left和right现在都要更新
    }
    else
      x_parent = y;  //如果前驱就是x的孩子，那就不用变了
    if (root == z)
      root = y;
    else if (z->parent->left == z)
      z->parent->left = y;   //和上面联系起来
    else
       z->parent->right = y;
    y->parent = z->parent;    
    __STD::swap(y->color, z->color);  //在这里要交换颜色，因为我们把z删了，用y去替换z，由于y顶替了z的位置，但我们不想让z位置平衡性改变，所以颜色还要保持z的颜色
    y = z;  //更新y                                 //那么就用swap把y的颜色换为原来z的颜色即可，不平衡的位置被转移到了y原有位置处                                          
...
//如果删除的结点不是红色，那么就需要平衡
  if (y->color != __rb_tree_red) { 
    while (x != root && (x == 0 || x->color == __rb_tree_black))  //x不能为根，x为黑色一直向上处理
	/***************************************   left   ****************************************/
      if (x == x_parent->left) {  
        __rb_tree_node_base* w = x_parent->right;
	//1111111111111111111111111111
	if (w->color == __rb_tree_red) {       //情况1，你叔颜色是红的，先通过颜色和旋转把你叔变成黑的
          w->color = __rb_tree_black;
          x_parent->color = __rb_tree_red;
          __rb_tree_rotate_left(x_parent, root);
          w = x_parent->right;           //你叔换人了!!!
        }
	//22222222222222222222222222222
        if ((w->left == 0 || w->left->color == __rb_tree_black) &&   //情况2，你叔两个儿子都是黑的或nil
            (w->right == 0 || w->right->color == __rb_tree_black)) {
          w->color = __rb_tree_red;
          x = x_parent;
          x_parent = x_parent->parent;   //直接把你叔变成红的，你向上从你爸开始继续检查
        } else {  //33333333333333444444444444，情况3，4都在else里面
          if (w->right == 0 || w->right->color == __rb_tree_black) {  //情况三，你叔右边的孩子是黑的,左孩子颜色不知道
            if (w->left) w->left->color = __rb_tree_black;
            w->color = __rb_tree_red;
            __rb_tree_rotate_right(w, root);   //把你叔变成红的，来个右转，把它转变为情况4
            w = x_parent->right;      //更新你叔
          }                                                                                               //说交换是因为情况4你叔右孩子是红的，你叔肯定是黑的。但是这里没用交换，效率更好   
          w->color = x_parent->color;        //情况4，你叔右孩子是红的，那好了，交换你叔(黑)和你爸(红),把你爸转到左边，你叔转到上边，你叔右孩子改为红的，
          x_parent->color = __rb_tree_black;                           //这下左半部分的删除的那个黑节点就平衡回来了。
