...
// Looks buggy
//¶Ô×Ö·û´®ÒÔ"/"·Ö¸î£¬½«Ã¿¸öµ¥´Êpush_back½øvectorÖÐ
...
Inspector::Inspector(EventLoop* loop,
                     ...
                     const string& name)
    : server_(loop, httpAddr, "Inspector:"+name),   //³õÊ¼»¯http·þÎñÆ÷
      processInspector_(new ProcessInspector),   //½ø³Ì¼ì²éÆ÷
      systemInspector_(new SystemInspector)    //ÏµÍ³¼ì²éÆ÷
{
  assert(CurrentThread::isMainThread());  //Ö»ÄÜÔÚÖ÷Ïß³É¹¹Ôì
  assert(g_globalInspector == 0);
  g_globalInspector = this;   //×¢²áÈ«¾Ö¹Û²ìÆ÷
  server_.setHttpCallback(boost::bind(&Inspector::onRequest, this, _1, _2));  //×¢²áhttp·þÎñÆ÷µÄ»Øµ÷º¯Êý
  processInspector_->registerCommands(this);   //×¢²áÃüÁî£¬Êµ¼ÊÉÏ¾ÍÊÇÌî³äÁ½¸ömap¶ÔÓ¦µÄÃüÁî£¬»Øµ÷º¯Êý£¬helpÐÅÏ¢
...
#endif
  //ÕâÑù×Ó×ö·¨ÊÇÎªÁË·ÀÖ¹¾ºÌ¬ÎÊÌâ
  //Èç¹ûÖ±½Óµ÷ÓÃstart£¬(µ±Ç°Ïß³Ì²»ÊÇloopËùÔÚµÄI/OÏß³Ì£¬ÊÇÖ÷Ïß³Ì)£¬ÄÇÃ´ÓÐ¿ÉÄÜ£¬µ±Ç°¹¹Ôìº¯Êý»¹Ã»·µ»Ø£¬
  //I/OÏß³Ì¿ÉÄÜÒÑ¾­ÊÕµ½ÁËhttp¿Í»§¶ËµÄÇëÇóÁË(ÒòÎªÕâÊÇº21 HttpServerÒÑ¾­Æô¶¯)£¬ÄÇÃ´¾Í»á»Øµ÷Inspector::onRequest£¬
  //¶øÕâÊ±ºò¹¹Ôìº¯Êý»¹Ã»·µ»Ø£¬Ò²¾ÍÊÇ¶ÔÏó»¹Ã»ÍêÈ«¹¹ÔìºÃ£¬»á³öÏÖcore dump
                    ...
                    const string& help)
{
  ...
  modules_[module][command] = cb;    //ÀûÓÃË«²ãmapµÄ²åÈëmodule¶ÔÓ¦µÄ»Øµ÷º¯Êý
  helps_[module][command] = help;     //Í¬Ñù²åÈëhelpÐÅÏ¢
}
...
{
  if (req.path() == "/")   //Èç¹ûÇëÇóÂ·¾¶Îª"/",
  {
    string result;
    MutexLockGuard lock(mutex_);
    //±éÀúhelps_£¬¸ñÊ½ÈçÏÂ(Ä£¿é£¬ÃüÁî£¬º¯Êý):
    // /proc/pid                  print pid 
         ...
         ++helpListI)
    {
      ...
      {
        result += "/";
        result += helpListI->first;  //Íâ²¿µü´úÆ÷£¬firstÊÇmodule£¬±ÈÈçÉÏÃæµÄproc
        result += "/";
        result += it->first;   //ÄÚ²¿µùÌ§ÆðµÚÒ»¸ö£¬firstÊÇcommand£¬±ÈÈçÉÏÃæµÄ
  ...
  {
    //Èç¹û²»ÊÇ¸ùÂ·¾¶£¬ÐèÒª¶ÔÇëÇóÂ·¾¶½øÐÐ·Ö¸î£¬µ÷ÓÃsplitº¯Êý£¬·Ö¸î"/"£¬½«µÃµ½µÄ×Ö·û´®±£´æÔÚresultÖÐ
    ...
    if (result.size() == 0)
    {
      //ÕâÖÖÇé¿öÊÇ´íÎóµÄ£¬ÒòÎªokÈÔÎªfalse
      LOG_DEBUG << req.path();
    }
    else if (result.size() == 1)
    {
      //Ö»ÓÐmoduleÃ»ÓÐcommand£¬·¢ËÍÒ»ÕÅÍ¼Æ¬,Ã²ËÆÊÇÈ«°×Í¼Æ¬
    ...
    {
      string module = result[0];   //²éÕÒÄ£¿é
      std::map<string, CommandList>::const_iterator commListI = modules_.find(module);
      if (commListI != modules_.end())
      {
        string command = result[1];   //result[1]¾ÍÊÇcommand
        const CommandList& commList = commListI->second;
        CommandList::const_iterator it = commList.find(command);   //ÔÚÃüÁîÁÐ±íÖÐ²éÕÒÃüÁî
        if (it != commList.end())
        {
          //Èç¹ûÓÐ¶îÍâµÄ²ÎÊý£¬Èç/proc/threads/1/2/3
          ...
          if (it->second)
          {
            ...
            resp->setStatusMessage("OK");
            resp->setContentType("text/plain");
            const Callback& cb = it->second;   //»Øµ÷º¯Êý
            resp->setBody(cb(req.method(), args));   //Ìå²¿°üº¬»Øµ÷º¯ÊýµÄ·µ»ØÖµ£¬ÊÇ×Ö·û´®
            ok = true;
          }
        }
      }
