...
void swoole_init(void)
{
    ...
    SwooleG.log_fd = STDOUT_FILENO;   //默认日志是标准输出
    SwooleG.cpu_num = sysconf(_SC_NPROCESSORS_ONLN);  //CPU数目
    SwooleG.pagesize = getpagesize();  //page大小
    SwooleG.pid = getpid();  //进程pid
    SwooleG.socket_buffer_size = SW_SOCKET_BUFFER_SIZE;   //  (8*1024*1024)
    ...
    //init global shared memory，swMemoryGlobal_new返回一个全局内存池构造器指针
    SwooleG.memory_pool = swMemoryGlobal_new(SW_GLOBAL_MEMORY_PAGESIZE, 1);  //申请一页带下内存
