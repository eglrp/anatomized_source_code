...
int swFactoryThread_create(swFactory *factory, int worker_num)
{
    ...
    factory->start = swFactoryThread_start;   //ÕâÀïÖ»ÊÇÉèÖÃstart»Øµ÷º¯Êý£¬¸Ã»Øµ÷º¯ÊýÊµ¼ÊÉÏÖ´ÐÐÑ­»·´´½¨ReactorÏß³Ì
...
static int swFactoryThread_start(swFactory *factory)
{
    ...
    swThreadPool_run(&object->workers);  //¸Ãº¯ÊýÄÚ²¿»áÑ­»·Ê¹ÓÃpthread_createº¯Êý´´½¨ReactorÏß³Ì
    ]
    return SW_OK;
}
