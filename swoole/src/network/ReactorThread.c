...
int swReactorThread_send(swSendData *_send)
{
    ...
    {
        conn = swServer_connection_verify(serv, session_id);   //获取conn
    }
    else
    {
    ...
    if (swBuffer_empty(conn->out_buffer))
    {
        ...
        {
            if (!conn->direct_send)   //如果不是直传，则需要将数据放入缓存。
            {
                goto buffer_send;
            }
    ...
    //close connection
    if (_send->info.type == SW_EVENT_CLOSE)  //如果发送长度为0，则指定trunk类型为SW_TRUNK_CLOSE(关闭连接)
    {
        trunk = swBuffer_new_trunk(conn->out_buffer, SW_CHUNK_CLOSE, 0);
        trunk->store.data.val1 = _send->info.type;
    }
    //sendfile to client
    else if (_send->info.type == SW_EVENT_SENDFILE) //如果发送的数据类型为sendfile，则调用swConnection_sendfile函数
    {
...
int swReactorThread_create(swServer *serv)
{
    ...
    else if (serv->factory_mode == SW_MODE_PROCESS)
    {
        ...
        //填充worker进程初始化函数，未来调用该函数去产生worker进程，其内部使用fork
...
#ifdef HAVE_CPU_AFFINITY   //设置CPU亲和度，绑定CPU
