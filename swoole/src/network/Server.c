...
{
    //ä»¥ç¬¬1ä¸ªä¸ºåŸºå‡†è¿›è¡ŒæŽ’åºï¼Œå–å‡ºæœ€å°å€¼
...
int swServer_master_onAccept(swReactor *reactor, swEvent *event)
{
    ...
    for (i = 0; i < SW_ACCEPT_MAX_COUNT; i++)
    {
        ...
        {
            reactor_id = new_fd % serv->reactor_num;   //round-roubin
        }
        ...
        if (serv->factory_mode != SW_MODE_SINGLE)
        {
            ...
            //ÔÚÕâÀï·Ö·¢¸ø×ÓÏß³Ì£¬Ò²¾ÍÊÇÌí¼Ó½øÈë×ÓÏß³ÌµÄsub_reactorÖ®ÖÐ
...
/**
 * proxyæ¨¡å¼
 * åœ¨å•ç‹¬çš„nä¸ªçº¿ç¨‹ä¸­æŽ¥å—ç»´æŒTCPè¿žæŽ¥
 */
static int swServer_start_proxy(swServer *serv)
{
    int ret;
    ...
    if (serv->onStart != NULL)   // ¹þ¹þ£¬ÕâÀïµ÷ÓÃonStart»Øµ÷º¯Êý
    {
        serv->onStart(serv);
    }
...
///////////////////////////////////////////////////////////////////////////////////////////////////
//ÔÚÕâÀïÃæ´´½¨ÁËmanage½ø³ÌÒÔ¼°worker½ø³Ì£¬ÉõÖÁ»¹ÓÐtask_worker½ø³Ì
//Ò»Ö±ÄÉÃÆfactory->startº¯ÊýÊ²Ã´Ê±ºò³õÊ¼»¯µÄ£¬Ô­À´ÊÇ½øÈë±¾º¯ÊýÖ®Ç°£¬ÔÚPHP_METHODÖÐ³õÊ¼»¯µÄ
    //factory start
...
void swServer_init(swServer *serv)
{
    ...
    serv->factory_mode = SW_MODE_BASE;   //Ä¬ÈÏÎªSW_MODE_BASE
...
int swServer_create(swServer *serv)
{
    ...
    {
        return swReactorThread_create(serv);   //Ä¬ÈÏÊÇSW_MODE_PROCESSÄ£ÐÍ
    }
}
