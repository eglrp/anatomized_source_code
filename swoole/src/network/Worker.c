...
void swWorker_onStart(swServer *serv)
{
    ...
    {
        serv->onWorkerStart(serv, SwooleWG.id);   //在这里调用onWorkerStart函数
    }
}
...
int swWorker_loop(swFactory *factory, int worker_id)
{
    ...
    swWorker_onStart(serv);  //内部会调用我们注册的onWorkerStart回调函数
...
#endif
    //因为main_reactor被fork过，所以在这里可以直接wait，可以和manager进程通信
    //不过回调函数是worker进程的回调函数，比如swWorker_onPipeReceive()，在上面注册过
    //main loop
