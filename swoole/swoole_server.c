...
int php_swoole_onReceive(swServer *serv, swEventData *req)
{
    ...
    //UDPä½¿ç”¨from_idä½œä¸ºport,fdåšä¸ºip
...
PHP_METHOD(swoole_server, __construct)
{
    ...
    long serv_port = 0;
    long serv_mode = SW_MODE_PROCESS;   //Ä¬ÈÏÊÇ¶à½ø³ÌÄ£Ê½
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////
...
#endif
    serv->factory_mode = serv_mode;  //Ä¬ÈÏÊÇSW_MODE_PROCESS£¬±¾º¯Êý¿ªÍ·ÓÐ¶¨Òå
    ...
    else
    {
        ...
        if (!port)
        {
            swoole_php_fatal_error(E_ERR OR, "listen server port failed.");
            return;
        }
    }
...
PHP_METHOD(swoole_server, set)
{
    ...
    if (SwooleGS->start > 0)  ////after swServer_start will set start=1
    {
    ...
    //½«ImmutableÊý×é·ÖÀë²¢ÖØÐÂ¹¹½¨Ò»¸ö·ÇImmutableÊý×é£¬¾Í¿ÉÒÔÐÞ¸ÄÕâ¸öÊý×éµÄÖµÁË¡£·ñÔòÓÉÓÚÔ­Êý×éÔÚphp´úÂëÖÐÓÉÓÚÃ»ÓÐ½øÐÐÐÞ¸Ä
    //±»±àÒëÆ÷ÓÅ»¯ÎªÖ»¶Á£¬ÔÚÀ©Õ¹ÖÐÖ±½ÓÐ´»á·¢ÉúÄÚ´æ´íÎó
    ...
    vht = Z_ARRVAL_P(zset);
    //chroot
    if (php_swoole_array_get_value(vht, "chroot", v))   //´Óset´«Èë²ÎÊýarrayÖÐ»ñÈ¡chrootÐÅÏ¢
    {
        convert_to_string(v);
