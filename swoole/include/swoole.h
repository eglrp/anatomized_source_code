...
typedef struct _swProtocol
{
    ...
    uint8_t package_eof_len;  //æ•°æ®ç¼“å­˜ç»“æŸç¬¦é•¿åº¦
    char package_eof[SW_DATA_EOF_MAXLEN + 1];  //æ•°æ®ç¼“å­˜ç»“æŸç¬¦

    char package_length_type;  //length field type
    uint8_t package_length_size;
    uint16_t package_length_offset;  //ç¬¬å‡ ä¸ªå­—èŠ‚å¼€å§‹è¡¨ç¤ºé•¿åº¦
    uint16_t package_body_offset;  //ç¬¬å‡ ä¸ªå­—èŠ‚å¼€å§‹è®¡ç®—é•¿åº¦
...
{
    int size;  //¹²ÏíÄÚ´æµÄ´óÐ¡
    char mapfile[SW_SHM_MMAP_FILE_LEN];   //ÓÃÀ´´æ´¢¹²ÏíÄÚ´æÊ¹ÓÃµÄÄÚ´æÓ³ÉäÎÄ¼þµÄÎÄ¼þÃû
    int tmpfd;  //ÄÚ´æÓ³ÉäÎÄ¼þµÄÃèÊö·û
    int key;    //shmÏµÁÐº¯Êý´´½¨µÄ¹²ÏíÄÚ´æµÄkeyÖµ
    int shmid;   //shmÏµÁÐº¯Êý´´½¨µÄ¹²ÏíÄÚ´æµÄid
    void *mem;   //ÓÃÀ´´æ´¢ËùÓÃÄÚ´æ
} swShareMemory;
...
{
    void *memory;   //ÄÚ´æÖ¸Õë£¬Ö¸ÏòÒ»Æ¬ÄÚ´æ¿Õ¼ä
    size_t size;    //ÄÚ´æ¿Õ¼ä´óÐ¡

    swFixedPool_slice *head;    //Á´±íÍ·²¿½Úµã
    swFixedPool_slice *tail;     //Á´±íÎ²²¿½Úµã£¬Á½¸öÖ¸ÕëÓÃÓÚ¿ìËÙ·ÃÎÊºÍÒÆ¶¯½Úµã

    /**
     * total memory size
     */
    uint32_t slice_num;   //½ÚµãÊýÄ¿

    /**
     * memory usage
     */
    uint32_t slice_use;   //ÒÑ¾­Ê¹ÓÃµÄÄÚ´æ½Úµã

    /**
     * Fixed slice size, not include the memory used by swFixedPool_slice
     */
    uint32_t slice_size;   //ÄÚ´æ½Úµã´óÐ¡

    /**
     * use shared memory
     */
    uint8_t shared;   //ÊÇ·ñ¹²ÏíÄÚ´æ
...
//ÓÃÓÚ·â×°´«µÝ¸øTheradµÄ²ÎÊý£¬void*Ö¸ÏòÁË²ÎÊýÄÚÈÝµÄµØÖ·£¬µÚ¶þ¸ö²ÎÊý±ê¼ÇÏß³ÌµÄid
...
struct _swReactor
{
    void *object;   //´æ·ÅµÄÊÇÊµ¼ÊReactorÄ£ÐÍµÄÄÚ´æµØÖ·
    void *ptr;  //reserve
    ...
    uint32_t event_num;   //´æ·ÅÏÖÓÐÊÂ¼þµÄÊýÄ¿
    uint32_t max_event_num;   //´æ·ÅÔÊÐí³ÖÓÐµÄ×î´óÊÂ¼þÊýÄ¿

    uint32_t check_timer :1;
    uint32_t running :1;   //±ê¼ÇÊÇ·ñÕýÔÚÔËÐÐ
    ...
    swReactor_handle handle[SW_MAX_FDTYPE];   //Ä¬ÈÏÊ±¼ä
    swReactor_handle write_handle[SW_MAX_FDTYPE];  //À©Õ¹ÊÂ¼þ(Ò»°ãÎªÐ´ÊÂ¼þ)
    swReactor_handle error_handle[SW_MAX_FDTYPE];  //À©Õ¹ÊÂ¼þ
    ...
    void (*onTimeout)(swReactor *);   //³¬Ê±»Øµ÷
    void (*onFinish)(swReactor *);  //Íê³ÉÒ»´ÎÂÖÑ¯»Øµ÷
...
typedef struct
{
    swTimer timer;   //È«¾Ö¶¨Ê±Æ÷

    uint8_t running :1;    //ÊÇ·ñÕýÔÚÔËÐÐ
    uint8_t use_timerfd :1;    //ÊÇ·ñÊ¹ÓÃLinuxÌá¹©µÄtimerfd¹¦ÄÜ
    uint8_t use_signalfd :1;   //ÊÇ·ñÊ¹ÓÃsignal_fd
    uint8_t reuse_port :1;      //¶Ë¿Ú¸´ÓÃ
     ...
     * Timer used pipe
     */
    uint8_t use_timer_pipe :1;  //timerÊÇ·ñÊ¹ÓÃ¹ÜµÀ

    int error;    //´íÎóÂë
    int process_type;   //±ê¼Çµ±Ç°½ø³ÌÀàÐÍ(manager or worker)
    pid_t pid;  //

    int signal_alarm;  //for timer with message queue
    int signal_fd;    //Ã»ÓÐÕÒµ½¸³ÖµÓï¾ä£¬Ó¦¸ÃÊÇÒÑ¾­·ÏÆú
    int log_fd;   //ÈÕÖ¾ÎÄ¼þµÄÃèÊö·û
    int null_fd;
    int debug_fd;
     ...
     */
    uint16_t task_worker_num;  //task worker½ø³ÌµÄÊýÁ¿
    char *task_tmpdir;
    uint16_t task_tmpdir_len;
    uint8_t task_ipc_mode;
    uint16_t task_max_request;

    uint16_t cpu_num;   //CPUµÄºËÊý

    uint32_t pagesize;   //µ±Ç°½ø³ÌµÄ·ÖÒ³´óÐÜ°¡
    uint32_t max_sockets;   //ÔÊÐíµÄ×î´ósocketfd
    ...
    swServer *serv;   //Ö¸ÏòswServer
    swFactory *factory;   //Ö¸ÏòswServerÀïµÄfactory

    swMemoryPool *memory_pool;   //ÄÚ´æ³Ø
    swReactor *main_reactor;   //È«¾Öreactor£¬ÔÚÖ÷½ø³ÌÖÐÖ»ÓÃÓÚ½ÓÊÜTCPÁ¬½Ó£¬ÔÚworker½ø³ÌÖÐ»¹ÐèÒª¶îÍâ´¦Àí¹ÜµÀµÄÊÂ¼þ

    swPipe *task_notify;   
    swEventData *task_result;

    pthread_t heartbeat_pidt;    //ÐÄÌøÏß³Ì£¬666

    char *dns_server_v4;   //Ó¦¸ÃÊÇÒì²½DNSÒ»Ð©Ö§³ÖµÄ¹¤×÷£¬Ä¿Ç°ÎÒ»¹Ã»ÓÐ½Ó´¥¹ý
    char *dns_server_v6;
