...
#define SW_HEARTBEAT_IDLE          0   //å¿ƒè·³å­˜æ´»æœ€å¤§æ—¶é—´
#define SW_HEARTBEAT_CHECK         0   //å¿ƒè·³å®šæ—¶ä¾¦æµ‹æ—¶é—´
...
{
    pthread_t thread_id;   //reactorÏß³Ìid
    swReactor reactor;     
    swUdpFd *udp_addrs;    //ÓÃÓÚ´¦ÀíudpÇëÇó
    swMemoryPool *buffer_input;  //Ö¸ÏòÒ»¸öÄÚ´æ³Ø
#ifdef SW_USE_RINGBUFFER
    int *pipe_read_list;
#endif
    swLock lock;
    int c_udp_fd;  //ÓÃÓÚ´¦ÀíudpÇëÇó
} swReactorThread;
...
typedef struct
{
    long target_worker_id;   //Ä¿±êworkerµÄid
    swEventData data;   //Êý¾Ý
} swDispatchData;

struct _swFactory
{
    void *object;   //ÓÃÓÚ´æ·ÅÒ»¸ö¾ßÌåµÄFactoryÀàÐÍ
    void *ptr; //server object
    int last_from_id;  //´æ·ÅÁË×î½üÒ»¸öÍ¨¹ý¸ÃFactory·¢ËÍÏûÏ¢µÄreactorµÄid
...
struct _swServer
{
     ...
     */
    uint16_t reactor_num;   //reactorÏß³ÌÊýÄ¿
    /**
     * worker process num
     */
    uint16_t worker_num;   //workerÏß³ÌÊýÄ¿
    /**
     * The number of pipe per reactor maintenance
     */
    uint16_t reactor_pipe_num;   //main reactorÏß³ÌµÄÊýÄ¿

    uint8_t factory_mode;   //factoryÄ£Ê½: baseÄ£Ê½£¬µ¥½ø³Ìµ¥Ïß³Ì¡¢¶à½ø³Ì¡¢¶àÏß³Ì

    uint8_t dgram_port_num;   //UDP¶Ë¿ÚºÅ

    /**
     * package dispatch mode
     */
    uint8_t dispatch_mode;   //·ÖÅäÄ£Ê½£¬1.Æ½¾ù·ÖÅä 2.°´fdÈ¡Ä£¹Ì¶¨·ÖÅä 3.Ê¹ÓÃÇÀÕ¼Ê½¶ÓÁÐ(IPC¶ÓÁÐ)·ÖÅä


    int worker_uid;    //woker½ø³ÌµÄuid
    int worker_groupid;  //worker groupid

    /**
     * max connection num
     */
    uint32_t max_connection;   //×î´óÁ¬½ÓÊýÄ¿

    /**
     * worker process max request
     */
    uint32_t max_request;   //×î´óÏìÓ¦ÊýÄ¿£¬Ê²Ã´¹í?

    int timeout_sec;   //ÕâÁ½¸öÊÇ³¬Ê±Ê±¼ä
    int timeout_usec;

    int sock_client_buffer_size; //clientµÄsocket»º³åÇøÉèÖÃ
    int sock_server_buffer_size; //serverµÄsocket»º³åÇøÉèÖÃ

    int signal_fd;   //Á½¸ö·ÏÆú±äÁ¿
    int event_fd;
    ...
    /*----------------------------Reactor schedule--------------------------------*/
    uint16_t reactor_round_i; //ÂÖÑ¯µ÷¶È
    uint16_t reactor_next_i; //Æ½¾ùËã·¨µ÷¶È
     ...
     */
    uint32_t daemonize :1;  //ÊÇ·ñ×÷Îªdaemon½ø³ÌÔËÐÐ
    /**
     * have udp listen socket
     */
    uint32_t have_udp_sock :1;  //ÓÐ¼àÌýUDPÌ×½Ó×Ö
    /**
     * have tcp listen socket
     */
    uint32_t have_tcp_sock :1;  //¼àÌýTCPÌ×½Ó×Ö
    /**
     * oepn cpu affinity setting
     */
    uint32_t open_cpu_affinity :1;   //CPUÇ×ºÍ¶È
    /**
    ...
    uint16_t heartbeat_idle_time; //ÐÄÌø´æ»îÊ±¼ä
    uint16_t heartbeat_check_interval; //ÐÄÌø¶¨Ê±Õì²âÊ±¼ä£¬±ØÐèÐ¡ÓÚheartbeat_idle_time
    ...
    swConnection *connection_list;  //Á¬½ÓÁÐ±í
...
//UDP, UDPå¿…ç„¶è¶…è¿‡0x1000000
//åŽŸå› ï¼šIPv4çš„ç¬¬4å­—èŠ‚æœ€å°ä¸º1,è€Œè¿™é‡Œçš„conn_fdæ˜¯ç½‘ç»œå­—èŠ‚åº
...
//ä½¿ç”¨connection_list[0]è¡¨ç¤ºæœ€å¤§çš„FD
...
//ä½¿ç”¨connection_list[1]è¡¨ç¤ºæœ€å°çš„FD
