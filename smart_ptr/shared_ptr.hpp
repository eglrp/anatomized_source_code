...
// sp_element, element_type     //其实就是T类型啊
...
//检查Y类型和T类型是否可以相互装换，这是shared_ptr要求的
    ...
    inline void sp_pointer_construct( boost::shared_ptr< T > * ppx, Y * p, boost::detail::shared_count & pn )
{
    ...
    */
    ...
    {
        boost::detail::sp_deleter_construct( this, p );  //初始化enable shared from this的weak_this_成员，是一个weak_ptr类型
        /*huangweiyang 
        template< class T, class Y > inline void sp_deleter_construct( boost::shared_ptr< T > * ppx, Y * p )
        {
            boost::detail::sp_enable_shared_from_this( ppx, p, p );
...
// ... except in C++0x, move disables the implicit copy   //除了在C++ 0x中，将禁用隐式复制
    //
    ...
    //注意可以使用Y类型，意味着只要可以转化为T类型就成。那么保存的基类就无需虚析构。
    template<class Y>
    ...
    //FIXME
    //注意这里用的auto_ptr的引用，并且没有调用auto_ptr的release函数
